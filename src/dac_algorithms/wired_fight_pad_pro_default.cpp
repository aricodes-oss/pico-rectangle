...
// Note: since the 1.0 coordinates on HID are defined by the highest value you
// reach, setting to +-80 should be fine
...
// MS is a modifier: MS+Start = Photo, MS+Left/Right/Up/Down = Dpad
// left/right/up/down
void actuateWFPPReport(GpioToButtonSets::F1::ButtonSet buttonSet) {
  ...
  USBConfigurations::WiredFightPadPro::hidReport = {
      ...
      .xStick = (uint8_t)(!(left ^ right) ? 128
                          : left          ? 48
                                          : 208), // Neutral SOCD
      .yStick = (uint8_t)(!(up ^ down) ? 128
                          : up         ? 48
                                       : 208), // Neutral SOCD + reversed axis
      .cxStick = (uint8_t)(!(buttonSet.cLeft ^ buttonSet.cRight) ? 128
                           : buttonSet.cLeft                     ? 48
                                             : 208), // Neutral SOCD
      .cyStick =
          (uint8_t)(!(buttonSet.cUp ^ buttonSet.cDown) ? 128
                    : buttonSet.cUp                    ? 48
                                    : 208), // Neutral SOCD + reversed axis
      .pad3 = 0};
}
