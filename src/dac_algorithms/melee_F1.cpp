...
GCReport getGCReport(GpioToButtonSets::F1::ButtonSet buttonSet) {
  ...
  if (vertical && horizontal) {
    if (bs.l || bs.r) {
      if (bs.mx == bs.my)
        xy = coords(0.7, readUp ? 0.7 : 0.6875);
      else if (bs.mx)
        xy = coords(0.6375, 0.375);
      else
        xy = (banParasolDashing && readUp) ? coords(0.475, 0.875)
                                           : coords(0.5, 0.85);
    } else if (bs.b && (bs.mx != bs.my)) {
      if (bs.mx) {
        if (bs.cDown)
          xy = coords(0.9125, 0.45);
        else if (bs.cLeft)
          xy = coords(0.85, 0.525);
        else if (bs.cUp)
          xy = coords(0.7375, 0.5375);
        else if (bs.cRight)
          xy = coords(0.6375, 0.5375);
        else
          xy = coords(0.9125, 0.3875);
      } else {
        if (bs.cDown)
          xy = coords(0.45, 0.875);
        else if (bs.cLeft)
          xy = coords(0.525, 0.85);
        else if (bs.cUp)
          xy = coords(0.5875, 0.8);
        else if (bs.cRight)
          xy = coords(0.5875, 0.7125);
        else
          xy = coords(0.3875, 0.9125);
      }
    } else if (bs.mx != bs.my) {
      if (bs.mx) {
        if (bs.cDown)
          xy = coords(0.7, 0.3625);
        else if (bs.cLeft)
          xy = coords(0.7875, 0.4875);
        else if (bs.cUp)
          xy = coords(0.7, 0.5125);
        else if (bs.cRight)
          xy = coords(0.6125, 0.525);
        else
          xy = coords(0.7375, 0.3125);
      } else {
        if (bs.cDown)
          xy = coords(0.3625, 0.7);
        else if (bs.cLeft)
          xy = coords(0.4875, 0.7875);
        else if (bs.cUp)
          xy = coords(0.5125, 0.7);
        else if (bs.cRight)
          xy = coords(0.6375, 0.7625);
        else
          xy = coords(0.3125, 0.7375);
      }
    } else
      xy = coords(0.7, 0.7);
  } else if (horizontal) {
    if (bs.mx == bs.my)
      ...
      xy = (buttonSet.left && buttonSet.right) ? coords(1.0, 0.0)
                                               : coords(0.6625, 0.0);
    else
      xy = ((banSlightSideB && bs.b) || buttonSet.left && buttonSet.right)
               ? coords(1.0, 0.0)
               : coords(0.3375, 0.0);
    ...
    if (bs.mx == bs.my)
      ...
      xy = coords(0.0, 0.5375);
    else
      xy = coords(0.0, 0.7375);
  } else {
    xy = coords(0.0, 0.0);
  }
...
// Since the C-stick code is rather tightly tied with the above block, we'll
// just overwrite the results here
#if USE_NUNCHUK
...
#if USE_NUNCHUK
  gcReport.cxStick = empty.x;
  gcReport.cyStick = empty.y;

  gcReport.dDown = bs.down;
  gcReport.dUp = bs.up;
  gcReport.dRight = bs.right;
  gcReport.dDown = bs.left;
#endif
