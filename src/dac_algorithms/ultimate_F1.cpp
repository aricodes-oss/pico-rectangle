...
GCReport getGCReport(GpioToButtonSets::F1::ButtonSet buttonSet) {
  ...
  if (left_wasPressed && bs.left && bs.right && !right_wasPressed)
    left_outlawUntilRelease = true;
  if (right_wasPressed && bs.left && bs.right && !left_wasPressed)
    right_outlawUntilRelease = true;
  if (up_wasPressed && bs.up && bs.down && !down_wasPressed)
    up_outlawUntilRelease = true;
  if (down_wasPressed && bs.up && bs.down && !up_wasPressed)
    down_outlawUntilRelease = true;
  ...
  if (left_outlawUntilRelease)
    bs.left = false;
  if (right_outlawUntilRelease)
    bs.right = false;
  if (up_outlawUntilRelease)
    bs.up = false;
  if (down_outlawUntilRelease)
    bs.down = false;
  ...
  if (vertical && horizontal) {
    if (bs.l || bs.r) {
      if (bs.mx == bs.my)
        xy = coords(0.7, 0.7); // Changed to 0.7, 0.7
      else if (bs.mx)
        xy = coords(0.6375, 0.375); // Untouched
      else
        xy = coords(0.85, 0.5); // Changed to 0.85, 0.5 (no parasol dashing)
    }
    // Removed extended up b coords
    else if (bs.mx != bs.my) { // All of these changed to match Crane's values
      if (bs.mx) {
        if (bs.cDown)
          xy = coords(0.7625, 0.6125);
        else if (bs.cLeft)
          xy = coords(0.825, 0.525);
        else if (bs.cUp)
          xy = coords(0.8875, 0.4375);
        else if (bs.cRight)
          xy = coords(0.9, 0.3375);
        else
          xy = coords(0.5, 0.325);
      } else {
        if (bs.cDown)
          xy = coords(0.6125, 0.7625);
        else if (bs.cLeft)
          xy = coords(0.525, 0.825);
        else if (bs.cUp)
          xy = coords(0.4375, 0.8875);
        else if (bs.cRight)
          xy = coords(0.3375, 0.9375);
        else
          xy = coords(0.475, 0.6125);
      }
    } else
      xy = coords(0.7, 0.7);
  } else if (horizontal) {
    if (bs.mx == bs.my)
      ...
      xy = coords(0.5, 0.0); // 0.6625 -> 0.5 + removed ledgedash facilitation
    else
      xy = coords(0.3375, 0.0); // No slight side B, unchanged besides that
  } else if (vertical) {
    if (bs.mx == bs.my)
      xy = coords(0.0, 1.0);
    else if (bs.mx)
      xy = coords(0.0, 0.2875); // 0.5375 -> 0.2875 (unlike Crane's code)
    else
      xy = coords(0.0, 0.6375); // 0.7375 -> 0.6375 <- very weird. To test.
  } else {
    xy = coords(0.0, 0.0);
  }

  if (horizontal && !readRight)
    xy.x = oppositeCoord(xy.x);
  if (vertical && !readUp)
    xy.y = oppositeCoord(xy.y);
  ...
  if (bs.mx && bs.my)
    ...
    cxy = bs.mx ? coords(0.8375, readUp ? 0.3125 : -0.3125) : coords(1.0, 0.0);
  else if (cVertical)
    cxy = coords(0.0, 1.0);
  else
    cxy = coords(0.0, 0.0);

  if (cHorizontal && bs.cLeft)
    cxy.x = oppositeCoord(cxy.x);
  if (cVertical && bs.cDown)
    cxy.y = oppositeCoord(cxy.y);
