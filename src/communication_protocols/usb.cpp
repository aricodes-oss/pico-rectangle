...
/* dev_lowlevel example, migrated to C++, with fewer bugs, extended with WCID &
 * multi-packet control transfer capabilities, and parameterized */
...
uint8_t
    ep0_buf[1024]; // Used to be 64. Sending a string of length > 32 in the
                   // string descriptor would cause undefined behaviour. Fun !
...
struct extended_compatibility_interface_descriptor {
  uint8_t
      firstInterfaceNumber; /**< Interface number for which an extended
                               compatibility feature descriptor is defined */
  uint8_t reserved1;
...
/**
 * @brief Same as above, but prepares answering an OS Descriptor request with a
 * //WinUSB usage query
 *
 * @return the length of the string descriptor in EP0 buf
 */
...
// We do buf(which can be ep0InRemainsBuffer)[i+64] -> ep0InRemainsBuffer[i] at
// the end of the startTransfer method
 ...
 * packet - 64 bytes)
 */
void usb_start_transfer(usb_endpoint_configuration *ep, uint8_t *buf,
                        uint16_t len) {
  ...
  //* Custom addition
  //* For in endpoints, if the transfer hasn't gone out yet, replace the
  //data_buffer, and don't flip the pid
  ...
  if (ep0InRemainingLength > 0 || !transferNotFired)
    ep->next_pid ^= 1u;
...
void usb_handle_device_descriptor(volatile usb_setup_packet *pkt) {
  ...
  for (int i = 0; i < sizeof(usb_device_descriptor); i++) {
    log_uart0_int(((uint8_t *)d)[i]);
    log_uart0(" ");
  }
  log_uart0("\n");
  usb_start_transfer(ep, (uint8_t *)d,
                     min(pkt->wLength, sizeof(usb_device_descriptor)));
}
...
void usb_handle_config_descriptor(volatile usb_setup_packet *pkt) {
  ...
  if (pkt->wLength >= d->wTotalLength) {
    ...
    if (useHID) {
      memcpy((void *)buf, dev_config.hid_descriptor,
...
void usb_handle_string_descriptor(volatile usb_setup_packet *pkt) {
  ...
  log_uart0("resulting string length is ");
  log_uart0_int(len);
  log_uart0("\n");
  usb_start_transfer(usb_get_endpoint_configuration(ep0_in_addr()), &ep0_buf[0],
                     len);
 ...
 * Two kind of requests will end up here: those that query the header only (16
 * long) and those that query the full thing (40 i.e 16 + 24*1) They are
 * indistinguishable except by length But it's just a question of truncation -
 * the same buffer start is to be used
 */
void usb_handle_extended_compatibility_descriptor(
    volatile usb_setup_packet *pkt) {
  log_uart0("usb_handle_extended_compatibility_descriptor\n");
  usb_start_transfer(usb_get_endpoint_configuration(ep0_in_addr()),
                     (uint8_t *)&extendedCompatibilityIdFeatureDescriptor,
                     min(sizeof(t_ext_comp_desc), pkt->wLength));
}
...
void usb_handle_hid_report_descriptor(volatile usb_setup_packet *pkt) {
  ...
  usb_start_transfer(usb_get_endpoint_configuration(ep0_in_addr()),
                     ...
                     min(hid_report_descriptor_len, pkt->wLength));
}
...
void log_usb_setup_packet(volatile usb_setup_packet *pkt) {
  ...
  log_uart0_int(pkt->bmRequestType);
  log_uart0(" ");
  log_uart0_int(pkt->bRequest);
  log_uart0(" ");
  log_uart0_int(pkt->wValue);
  log_uart0(" ");
  log_uart0_int(pkt->wIndex);
  log_uart0(" ");
  log_uart0_int(pkt->wLength);
  log_uart0(" ");
  log_uart0("\n");
}
...
void usb_handle_setup_packet(void) {
  volatile usb_setup_packet *pkt =
      (volatile usb_setup_packet *)&usb_dpram->setup_packet;
  ...
  if (useWinUSB && req_direction == USB_DIR_IN &&
    ...
    if (req == USB_REQUEST_SET_ADDRESS) {
      ...
      usb_start_transfer(usb_get_endpoint_configuration(ep0_in_addr()), NULL,
                         0);
...
void ep0_in_handler(uint8_t *buf, uint16_t len) {
  ...
  if (ep0InRemainingLength > 0) {
    ...
    log_uart0("\n");
    usb_start_transfer(usb_get_endpoint_configuration(ep0_in_addr()),
                       ep0InRemainsBuffer, ep0InRemainingLength);
...
void ep_out_handler(uint8_t *buf, uint16_t len) {
  ...
  usb_start_transfer(usb_get_endpoint_configuration(ep_out_addr()), NULL,
                     5); // TODO Do something about the 5
}
...
void usb_device_init() {
  // TODO Look into why there's so much delay for the device to be recognized
  // when plugged in
...
void enterMode(Configuration config, int headroomUs) {
  ...
  // Always same size, always usb_dt, always interrupt, always bInterval1, the
  // variables here are whether the 2nd endpoint is id 1 or 2, and the max
  // packet sizes
  ep_in = {
  ...
  ep_out = {
      ...
      .bDescriptorType = USB_DT_ENDPOINT,
      .bEndpointAddress =
          config.epOutId, // Out endpoint (host to device), ID 1 or 2
      .bmAttributes = USB_TRANSFER_TYPE_INTERRUPT,
  ...
  // Parameters here are the bcdHID, the HID descriptor, and the HID descriptor
  // length, plus whether we're using HID (could be inferred, TODO later)
  useHID = config.hid;
  hid_descriptor = {.bLength = 0x09,
  ...
  device_descriptor = {
      ...
      .bcdUSB = 0x0200,  //* USB 2.0 device (actually 1.1 but 2.0 identification
                         //necessary for WCID)
      ...
      .iSerialNumber = 3, // Serial number index (used by some softwares as the
                          // device name... somehow)
  ...
  interface_descriptor = {
      ...
      .bInterfaceClass =
          (config.hid ? (uint8_t)0x03
                      : (uint8_t)0xff), // 3 = HID class, 255 = Vendor class
      .bInterfaceSubClass = 0,
      .bInterfaceProtocol = 0,
      .iInterface = 0};

  // The variable here is the size, that depends on whether we have a HID
  // descriptor or not
  config_descriptor = {
  ...
  usb_start_transfer(
      usb_get_endpoint_configuration(ep_out_addr()), nullptr,
      config.outEpMaxPacketSize); // Get ready to receive something and do
  ...
  while (1) {
    ...
    while (!ep1_in_handler_happened)
      ; // TODO More clever algorithm ?
    ep1_in_handler_happened = false;

    // Wait until n us before 1000-x us after completion of the previous ep1
    // transfer
    ...
    usb_start_transfer(usb_get_endpoint_configuration(ep_in_addr()),
                       config.hidReportPtr, config.inEpActualPacketSize);
  }
}
