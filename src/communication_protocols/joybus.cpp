...
#define accessBit(buffer, offset)                                              \
  ((buffer[offset / 8] & (0x0080 >> (offset % 8))) != 0)
...
uint8_t responseBuffer[10] = {0x00, 0x80, 0x80, 0x80, 0x80,
...
stateLabel_InputAwaitHigh:
  ...
  readIndex++;
  if (readIndex == 100)
    goto stateLabel_InputInit; // shields against garbage data on gc line

  // Look for bit patterns

  if (readIndex == 9 && (readings[8] == true) && (readings[0] == false) &&
      (readings[2] == false) && (readings[3] == false) &&
      (readings[4] == false) && (readings[5] == false) &&
      (readings[6] == false)) {
  ...
  if (readIndex == 25 && (readings[24] == true) && (readings[0] == false) &&
...
stateLabel_InputAwaitLow:
  ...
  while (sio_hw->gpio_in & (1 << gcDataPin)) {
    if ((systick_hw->cvr - target) & 0x00800000)
...
void respond(uint8_t *responsePointer, uint32_t responseBitLength) {
  ...
  // When we switch from input pull-up to output-high, we don't want a sudden
  // low Setting high before setting the direction adresses that
  ...
  for (uint32_t i = 0; i < responseBitLength; i++) {
    ...
    target =
        origin - i * 4 * us - (accessBit(responsePointer, i) ? us : 3 * us);
    while ((target - systick_hw->cvr) & 0x00800000)
      ;
    changeBit(sio_hw->gpio_out, gcDataPin, 1);
  }
...
void respondToPoll(GCReport gcReport) { respond((uint8_t *)&gcReport, 64); }

/* The Pico's speed, combined with the speed of this function (not checking
SDI/pivot helps) makes it so building the report is so fast, if we don't answer
ASAP, we'll start answering before the console is done asking. Answering
meaning, pulling the line when they're pulling
it too. This could be bad. So we delay starting to build the response state
until we're ready to talk - that is, by 2us. */
void wait2Us() {
