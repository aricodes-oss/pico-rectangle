...
uint8_t WFPP_hid_report_descriptor[WFPP_hid_report_descriptor_len] = {
    5,   1,   9,   5,   161, 1,   21, 0,   37, 1,   53,  0,   69,  1,
    117, 1,   149, 14,  5,   9,   25, 1,   41, 14,  129, 2,   149, 2,
    129, 1,   5,   1,   37,  7,   70, 59,  1,  117, 4,   149, 1,   101,
    20,  9,   57,  129, 66,  101, 0,  149, 1,  129, 1,   38,  255, 0,
    70,  255, 0,   9,   48,  9,   49, 9,   50, 9,   53,  117, 8,   149,
    4,   129, 2,   117, 8,   149, 1,  129, 1,  192};

const uint16_t descriptor_strings_len = 3;
const char *descriptor_strings[descriptor_strings_len] = {
    "Performance Designed Products", "Wired Fight Pad Pro for Nintendo Switch",
    "00002014603B7353" // Release number
};

uint8_t hatFromDpad(const GCReport &gcReport) {
  if (gcReport.dUp && !gcReport.dLeft && !gcReport.dRight)
    return 0;
  else if (gcReport.dUp && gcReport.dRight)
    return 1;
  else if (gcReport.dRight && !gcReport.dDown)
    return 2;
  else if (gcReport.dRight && gcReport.dDown)
    return 3;
  else if (gcReport.dDown && !gcReport.dLeft)
    return 4;
  else if (gcReport.dDown && gcReport.dLeft)
    return 5;
  else if (gcReport.dLeft && !gcReport.dUp)
    return 6;
  else if (gcReport.dLeft && gcReport.dUp)
    return 7;
  else
...
/*
   ...
   State,No Null Position) 0x95, 0x02,        //   Report Count (2) 0x81, 0x01,
   //   Input (Const,Array,Abs,No Wrap,Linear,Preferred State,No Null Position)
        ...
        0x65, 0x14,        //   Unit (System: English Rotation, Length:
   Centimeter) 0x09, 0x39,        //   Usage (Hat switch) 0x81, 0x42,        //
   Input (Data,Var,Abs,No Wrap,Linear,Preferred State,Null State) 0x65, 0x00, //
   Unit (None) 0x95, 0x01,        //   Report Count (1) 0x81, 0x01,        //
   Input (Const,Array,Abs,No Wrap,Linear,Preferred State,No Null Position) 0x26,
   0xFF, 0x00,  //   Logical Maximum (255) 0x46, 0xFF, 0x00,  //   Physical
   Maximum (255) 0x09, 0x30,        //   Usage (X) 0x09, 0x31,        //   Usage
   (Y) 0x09, 0x32,        //   Usage (Z) 0x09, 0x35,        //   Usage (Rz)
        0x75, 0x08,        //   Report Size (8)
        0x95, 0x04,        //   Report Count (4)
        0x81, 0x02,        //   Input (Data,Var,Abs,No Wrap,Linear,Preferred
   State,No Null Position) 0x75, 0x08,        //   Report Size (8) 0x95, 0x01,
   //   Report Count (1) 0x81, 0x01,        //   Input (Const,Array,Abs,No
