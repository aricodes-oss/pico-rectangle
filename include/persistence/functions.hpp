...
inline bool isnt0xFF(uint32_t word) { return word != 0xFF; }

template <typename Page> const Page *read() {
  static_assert(sizeof(Page) <= FLASH_SECTOR_SIZE);
  return (Page *)(XIP_BASE + FLASH_OFFSET + Page::index * FLASH_SECTOR_SIZE);
}

template <typename Page> Page clone() {
  static_assert(sizeof(Page) <= FLASH_SECTOR_SIZE);
  Page copy =
      *(Page *)(XIP_BASE + FLASH_OFFSET + Page::index * FLASH_SECTOR_SIZE);
  return copy;
}
...
template <typename Page> void commitPtr(const Page *ptrToPage) {
  static_assert(sizeof(Page) <= FLASH_SECTOR_SIZE);
  uint32_t ints = save_and_disable_interrupts();
  uint8_t page[FLASH_SECTOR_SIZE];
...
template <typename Page> void commit(const Page &ptrToPage) {
  commitPtr(&ptrToPage);
}
